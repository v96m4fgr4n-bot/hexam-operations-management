...
function initializeSpreadsheet() {
  ...
  ensureSheetWithHeaders_(ss, 'Expenses', [
    'ExpenseId', 'ExpenseDate', 'Category', 'Description', 'Amount', 'CreatedBy', 'CreatedAt'
  ]);

  ensureSheetWithHeaders_(ss, 'AuditLog', [
    'LogId', 'Timestamp', 'UserEmail', 'Action', 'EntityType', 'EntityId', 'Summary'
  ]);
...
function seedDefaultSettings_(sheet) {
  var defaults = [
    ...
    ['BRICK_PRICE_PER_1000', 100, 'Price charged to the client per 1000 bricks, when a trip includes bricks (not just delivery)'],
